...
vector<int> jimOrders(vector<vector<int>> orders) {
    int n=orders.size();//finding Number of rows
    
    //creates an array that consists of both values and indices
    vector<pair<int, int> > st;
    vector<int> op;
    for(int i=0;i<n;i++){
        st.push_back(make_pair((orders[i][0]+orders[i][1]),i));
    }
    sort(st.begin(),st.end());//sorting elements in descending order
    for(int i=0;i<n;i++){
    op.push_back(st[i].second+1);
    }
    return op;
}
